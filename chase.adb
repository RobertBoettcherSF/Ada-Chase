...
package body Chase is
   ...
   function Create_Literal (Text : String) return Value is
      V : Value := (others => <>);
   begin
      if Text'Length > 10 then raise Invalid_FD with "Value text too long"; end if;
   ...
   function Find_Attribute_Index (Attributes : Attribute_List; Attr_Length : Integer; Attr : Attribute) return Integer is
   begin
      for I in 1..Attr_Length loop
         if Attributes(I) = Attr then return I; end if;
      end loop;
      return -1;
   end Find_Attribute_Index;

   function Attribute_In_Schema (Attributes : Attribute_List; Attr_Length : Integer; Attr : Attribute) return Boolean is
   begin
   ...
   procedure Create_Initial_Tableau (Original_Tuple : Tuple; Decomp : Decomposition; Decomp_Length : Integer;
                                    Tuple_Length : Integer; Result : out Tableau; Result_Length : out Integer) is
   begin
   ...
   procedure Apply_FD (T : in out Tableau; T_Length : Integer; FD : Functional_Dependency; Tuple_Length : Integer; Changed : out Boolean) is
   ...
   begin
      ...
      if Right_Pos = -1 then raise Invalid_FD with "Right-hand attribute not found"; end if;
         ...
         for Row2 in Row1+1..T_Length loop
            ...
            begin
               for I in 1..FD.Left_Length loop
                  ...
                  begin
                     if Pos = -1 then Left_Match := False; exit; end if;
                     if not Values_Equal(T(Row1)(Pos), T(Row2)(Pos)) then Left_Match := False; exit; end if;
               ...
               if Left_Match and Has_Literal and not Values_Equal(T(Row1)(Right_Pos), T(Row2)(Right_Pos)) then
                  if T(Row1)(Right_Pos).V_Kind = Literal_Value then
                     T(Row2)(Right_Pos) := T(Row1)(Right_Pos); Changed := True;
                  elsif T(Row2)(Right_Pos).V_Kind = Literal_Value then
                     T(Row1)(Right_Pos) := T(Row2)(Right_Pos); Changed := True;
                  else
   ...
   function Contains_Original_Tuple (T : Tableau; T_Length : Integer; Original_Tuple : Tuple; Tuple_Length : Integer) return Boolean is
   begin
   ...
   function Standard_Chase (Original_Tuple : Tuple; FDs : FD_List; FDs_Length : Integer;
                          Decomp : Decomposition; Decomp_Length : Integer; Tuple_Length : Integer) return Boolean is
      Tableau : Tableau;
   ...
   begin
      ...
      for Iteration in 1..100 loop
         ...
         if Contains_Original_Tuple(Tableau, Tableau_Length, Original_Tuple, Tuple_Length) then return True; end if;
      end loop;
   ...
   begin
      for I in 1..FDs_Length loop
         if FDs(I).Left_Length = 0 then return False; end if;
      end loop;
      return True;
   end Validate_FDs;

   function Validate_Decomposition (Decomp : Decomposition; Decomp_Length : Integer;
                                    All_Attrs : Attribute_List; All_Attrs_Length : Integer) return Boolean is
   begin
